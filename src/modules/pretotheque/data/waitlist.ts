...
import { busyRanges, canBook } from "@/modules/pretotheque/domain/availability";
 ...
 * "the request would actually succeed".
 */
export async function notifyWaitlistIfFreed(itemId: string): Promise<void> {
  ...
  const existingBookings = await db.select().from(booking).where(eq(booking.itemId, itemId));
