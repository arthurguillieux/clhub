 ...
 * not application code, is what makes double-booking a single item impossible.
 * See docs/02-architecture.md §3.3 and ADR-006.
 */
...
describe("booking exclusion constraint", () => {
  ...
  it("rejects an overlapping approved booking, but allows pending overlaps and adjacent bookings", async () => {
    await db
      .insert(booking)
      .values({ itemId, borrowerId: memberId, startDate: "2026-08-01", endDate: "2026-08-05", status: "approved" });
  ...
  it("under real concurrency, exactly one of two overlapping approved inserts succeeds", async () => {
    const attempt = (startDate: string, endDate: string) =>
      db
        .insert(booking)
        .values({ itemId, borrowerId: memberId, startDate, endDate, status: "approved" })
        .then(() => "ok" as const)
